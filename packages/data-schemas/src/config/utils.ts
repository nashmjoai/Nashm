 ...
 * Priority:
 * 1. Nashm_LOG_DIR environment variable
 * 2. If running within Nashm monorepo (when cwd ends with /api), use api/logs
...
export const getLogDirectory = (): string => {
  if (process.env.Nashm_LOG_DIR) {
    return process.env.Nashm_LOG_DIR;
  }
  ...
  // For Nashm project structure, use api/logs
  // For external consumers, they should set Nashm_LOG_DIR
  if (cwd.includes('Nashm')) {
    return apiLogsPath;
  }
