...
export interface HostedTool {
  type: string; // e.g., 'Nashm:web_search'
  [key: string]: unknown;
}
...
/** Attachment content types for Nashm extensions */
export interface NashmAttachmentContent {
...
/**
 * Nashm attachment event - custom streaming event for file/image attachments
 * Follows Open Responses extension pattern with Nashm: prefix
 */
export interface NashmAttachmentEvent extends BaseEvent {
  type: 'Nashm:attachment';
  /** The attachment data */
  attachment: NashmAttachmentContent;
...
/** Union of all streaming events (including Nashm extensions) */
export type ResponseEvent =
  ...
  | ReasoningDoneEvent
  | ErrorEvent
  // Nashm extensions (prefixed per Open Responses spec)
