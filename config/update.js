...
(async () => {
  ...
  if (docker) {
    ...
    console.purple('Pruning all Nashm Docker images...');

    const imageName = singleCompose ? 'Nashm_single' : 'Nashm';
    try {
      execSync(`${sudo}docker rmi ${imageName}:latest`, { stdio: 'inherit' });
    } catch (e) {
      console.purple('Failed to remove Docker image Nashm:latest. It might not exist.');
    }
    console.purple('Removing all unused dangling Docker images...');
    execSync(`${sudo}docker image prune -f`, { stdio: 'inherit' });
    console.purple('Building new Nashm image...');
