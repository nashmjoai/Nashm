...
import { dataService, MutationKeys, QueryKeys, defaultOrderQuery } from 'nashm-data-provider';
import {
  Constants,
...
// Support and Admin Console Mutations
export const useCreateSupportTicketMutation = (
  options?: any,
): UseMutationResult<any, unknown, { subject: string; message: string }, unknown> => {
  return useMutation(
    (payload) => dataService.createSupportTicket(payload),
    options,
  );
};

export const useUpsertAdminSubscriptionMutation = (
  options?: any,
): UseMutationResult<any, unknown, any, unknown> => {
  const queryClient = useQueryClient();
  return useMutation(
    (payload) => dataService.upsertAdminSubscription(payload),
    {
      ...options,
      onSuccess: (data, variables, context) => {
        queryClient.invalidateQueries(['adminConsoleUsers']);
        options?.onSuccess?.(data, variables, context);
      },
    },
  );
...
export const useUpdateAdminConsoleModelMutation = (
  options?: any,
): UseMutationResult<any, unknown, any, unknown> => {
  const queryClient = useQueryClient();
  return useMutation(
    (payload) => dataService.updateAdminConsoleModel(payload),
    {
      ...options,
      onSuccess: (data, variables, context) => {
...
export const useUpdateAdminConsoleTicketMutation = (
  options?: any,
): UseMutationResult<any, unknown, { id: string; status: string }, unknown> => {
  const queryClient = useQueryClient();
  return useMutation(
    ({ id, status }: { id: string; status: string }) => dataService.updateAdminConsoleTicket(id, status),
    {
      ...options,
      onSuccess: (data, variables, context) => {
        queryClient.invalidateQueries(['adminConsoleTickets']);
        options?.onSuccess?.(data, variables, context);
      },
    },
  );
...
export const useAddFamilyMemberMutation = (
  ...
  const queryClient = useQueryClient();
  return useMutation(
    (email: string) => dataService.addFamilyMember(email),
    {
      ...options,
      onSuccess: (data, variables, context) => {
...
export const useRemoveFamilyMemberMutation = (
  ...
  const queryClient = useQueryClient();
  return useMutation(
    (userId: string) => dataService.removeFamilyMember(userId),
    {
      ...options,
      onSuccess: (data, variables, context) => {
        queryClient.invalidateQueries(['familyPlan']);
