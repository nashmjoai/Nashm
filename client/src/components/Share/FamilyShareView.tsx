...
import { buildTree } from 'nashm-data-provider';
import { CalendarDays, Settings } from 'lucide-react';
import { useGetFamilyPlanMemberConversationMessagesQuery } from '~/data-provider';
import {
  Spinner,
  Button,
  OGDialog,
...
function FamilyShareView() {
  const localize = useLocalize();
  const { data: config } = useGetStartupConfig(undefined, { context: 'share' });
  const { theme, setTheme } = useContext(ThemeContext);
  const { userId, conversationId } = useParams();
  const { data, isLoading } = useGetFamilyPlanMemberConversationMessagesQuery(
    userId ?? '',
    conversationId ?? '',
  );
  ...
  let docTitle = '';
  if (config?.appTitle != null && data?.conversation?.title != null) {
    docTitle = `${data.conversation.title} | ${config.appTitle}`;
  } else {
    docTitle = data?.conversation?.title ?? config?.appTitle ?? document.title;
  }
  ...
  const formattedDate =
    data?.conversation?.createdAt != null
      ? new Date(data.conversation.createdAt).toLocaleDateString(locale, {
          month: 'long',
          day: 'numeric',
          year: 'numeric',
        })
  ...
  return (
    <ShareContext.Provider value={{ isSharedConvo: true, shareId: conversationId }}>
...
export default memo(FamilyShareView);
