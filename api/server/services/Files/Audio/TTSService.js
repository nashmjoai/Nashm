...
const { genAzureEndpoint, logAxiosError, applyAxiosProxyConfig } = require('@nashm/api');
const { extractEnvVariable, TTSProviders } = require('nashm-data-provider');
...
class TTSService {
  ...
  getProvider(appConfig) {
    ...
    if (!ttsSchema) {
      throw new Error(
        'No TTS schema is set. Did you configure TTS in the custom config (Nashm.yaml)?',
      );
    }
