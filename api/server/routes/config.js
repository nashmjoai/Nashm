...
const { EModelEndpoint, defaultSocialLogins } = require('nashm-data-provider');
const { logger, getTenantId, SystemCapabilities } = require('@nashm/data-schemas');
...
function buildPreLoginPayload() {
  ...
  const payload = {
    appTitle: process.env.APP_TITLE || 'Nashm',
...
function buildPostLoginPayload() {
  ...
  const payload = {
    ...
    helpAndFaqURL: process.env.HELP_AND_FAQ_URL || 'https://Nashm.ai',
    sharedLinksEnabled,
...
router.get('/', async function (req, res) {
  try {
    ...
    const payload = {
      ...
      modelSpecs: sanitizeModelSpecs(appConfig?.modelSpecs, endpointsConfig),
